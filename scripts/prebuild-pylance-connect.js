...
function findPylanceLog(serverDataDir) {
    ...
    var sessionDirs = fs.readdirSync(logsDir).reverse();
    for (var i = 0; i < sessionDirs.length; i++) {
        var pylanceLog = path.join(
            logsDir,
            sessionDirs[i],
            "exthost1",
...
async function main() {
    log("Connecting to VS Code web at " + TARGET_URL);
    log("Server data dir: " + SERVER_DATA_DIR);
